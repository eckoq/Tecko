...
#pragma pack()
	...
	public:
		...
		//将CHUNK中的数据分片进行组合
		int merge(BC_DISK_HANDLER chunk_node_hdr, int chunk_len, void* data_buf, int* data_len);
		...
		static long get_pool_size(int n_chunks)
		{
			long chunk_total_size = (long)n_chunks * (long)get_chunk_node_size();
