...
int CStat::TimeStat_End(string row_name, unsigned seq)	//seq 号要唯一
{
	...
	struct timeval t;
	gettimeofday(&t, NULL);
	unsigned spend = 1000 * (t.tv_sec - start.tv_sec) + (t.tv_usec - start.tv_usec) / 1000;	//3位有效数字，微秒
...
void CStat::Print_Header()
{
	...
	len = sprintf(line, "%23s", "");
	len1 = sprintf(line1, "%-23s", "total:");
	len2 = sprintf(line2, "%-23s", "count /1s:");
	...
	for(i=0; i<size; i++)
	{
		...
		len += sprintf(line + len,   "|%9s ", name.c_str());
		len1 += sprintf(line1 + len1, "|%9u ", value);
		len2 += sprintf(line2 + len2, "|%9u ", value  / time_gap);
	}
...
void CStat::Print_Body()
{
	...
	line[0] = 0;
	line_len = sprintf(line, "%-23s", "");
	for(j=0; j<col_size; j++)
	{
		line_len += sprintf(line + line_len, "|%8s ", v_body_col_items[j].c_str());
	}
	
	line_len += sprintf(line + line_len, "|%8s |%8s |%8s | >%0.3fs |\n", 
										"tcount",
										"avg_de_s",
										"de_max_s",
										_delay_up/1000.0);
	...
	for(i=0; i<row_size; i++)
	{
		...
		line_len = sprintf(line, "%-23s", ((string)name + ":").c_str());
		...
		for(j=0; j<col_size; j++)
		{
			...
			line_len += sprintf(line + line_len, "|%8d ", value);
		}
		...
		line_len += sprintf(line + line_len, "|%8u |%8.3f |%8.3f |%8d |\n", 
									tcount,
									avg/ 1000.0,
									max/ 1000.0,
									up);
