...
namespace tfc
{
	namespace net
	{
		...
		public:
			...
			inline bool socket_is_ok() const {return (_socket_fd != INVALID_SOCKET);}
			inline void detach(){_close_protect = false;}
			inline void attach(int fd){if (_close_protect){close();_close_protect = true;} _socket_fd = fd;}
			...
			int listen(bool defer_accept = true, int backlog = 1024);
