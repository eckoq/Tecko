...
void CFileConfig::GetConfigMap(std::map<std::string, std::string>* config_vars) const
{
    map<string, string>::const_iterator subiter;
    map<string, map<string, string> >::const_iterator iter;

    for (iter = _pairs.begin(); iter != _pairs.end(); ++iter)
    {
        const std::string& path = iter->first;
        const std::map<std::string, std::string>& submap = iter->second;

        for (subiter = submap.begin(); subiter != submap.end(); ++subiter)
        {
            string key = path + "\\" + subiter->first;
            string value = subiter->second;
