...
int CacheAccess::set(const char* key, const char* data, unsigned data_len,ENodeFlag flag/*=NODE_FLAG_DIRTY*/, bool need_sync/*=false*/)
{
	...
	if ((_cache_dump_min >= 0) && (flag == NODE_FLAG_DIRTY))
	{
		...
		if (_binlog.WriteToBinLog(szBuff,bufflen,need_sync) != 0)
		{
			return 1;
		}
	}
...
int CacheAccess::set(const char* key, const char* data, unsigned data_len, long expiretime, ENodeFlag flag/*=NODE_FLAG_DIRTY*/, bool need_sync/*=false*/)
{
	//	find
	TMBHashKey hash_key;
	memcpy(hash_key._.md5_, key, TMBHashKey::C_DATA_LEN);
...
int CacheAccess::core_dump(char *szcorefile)
{
	...
	for(CacheAccess::dirty_iterator it = begin(); it != end();it++)
	{
		...
		if (buf_size < (write_len + TMBHashKey::C_DATA_LEN + sizeof(unsigned) + data_len))
		{
			...
			if (fwrite(buf, write_len, 1, fp) != 1)
			{
				fclose(fp);
				delete[] buf;
				delete[] node_buf;
...
int CacheAccess::mirror_dump(char *mirror_file)
{
	...
	if (fwrite(_mem, _mem_size, 1, fp) != 1)
    {
        fclose(fp);
        return -1;
    }
