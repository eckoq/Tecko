...
namespace tfc{namespace diskcache
{
	...
	public:
...
#else
		int set(const char* key, const char* data, unsigned data_len, ENodeFlag flag=NODE_FLAG_DIRTY, bool need_sync=false);
		int set(const char* key, const char* data, unsigned data_len, long expiretime, ENodeFlag flag=NODE_FLAG_DIRTY, bool need_sync=false);
#endif
		...
		mydiskalloc_chunk* get_disk_chunk(const char* key);
		...
		int mark_clean(int modbase, int mobres, bool need_sync=false);
	...
	public:
		...
		int set(unsigned uin, const char* data, unsigned data_len,ENodeFlag flag=NODE_FLAG_DIRTY, bool need_sync=false);
		int del(unsigned uin, bool need_sync=false);
		...
		int mark_clean(unsigned uin, bool need_sync=false);
