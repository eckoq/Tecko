...
int CHashMap::open(char* pool, const std::string& file_path, bool init, int node_total, int bucket_size, unsigned long long filesize, unsigned minchunksize)
{
	int ret = 0;
...
//	if ((ret = allocator_.open(pool_tail_, file_path, init, n_chunks, chunk_size)) != 0) 
	...
	if ((ret = mydiskalloc_init(&myhead_, pool_tail_, mydiskalloc_calcsize(filesize, minchunksize), init, file_path.c_str(), filesize, minchunksize)) != 0)
#else
	if ( ( ret = mydiskalloc_init(&myhead_, pool_tail_, mydiskalloc_calcsize((unsigned)node_total), init, file_path.c_str(), filesize, minchunksize, (unsigned)node_total) ) != 0 )
#endif
	{
		return ret;
	}
...
THashNode* CHashMap::find_node_rdonly(TMBHashKey &key)
{
...
int CHashMap::data_dump(const char *dump_name)
{
	return mydiskalloc_data_dump(myhead_, dump_name);
}

int CHashMap::data_recover(const char *dump_name, char *bak_name)
{
	return mydiskalloc_data_recover(myhead_, dump_name, bak_name);
}

mydiskalloc_chunk* CHashMap::node_2_chunk(THashNode *node)
{
	return get_chunk_extern(myhead_, node->chunk_head_);
}
...
#else
	DISK_HANDLE head_hdr = mydiskalloc_alloc(myhead_, new_len);
	if(head_hdr == NULL_HANDLE) {
		free_list_insert(node);
		return NULL;	
	}
	if(mydiskalloc_write(myhead_, head_hdr, new_data, new_len) != 0) {
		mydiskalloc_free(myhead_, head_hdr);
		free_list_insert(node);
...
THashNode* CHashMap::insert_node_metadata(TMBHashKey &key, int new_len, long expiretime)
{
	THashNode* node = free_list_remove();
	if (node == NULL) 
	{
...
THashNode* CHashMap::update_node(THashNode* node, void* new_data, int new_len, char* old_data, int* old_len)
{
	if(old_data != NULL && old_len != NULL)
	{
		...
		*old_len = node->chunk_len_;
		if(mydiskalloc_read(myhead_, node->chunk_head_, old_data, node->chunk_len_) != 0)
		{
			return NULL;
		}
	}
...
#else
	/*
	if(new_len > node->chunk_len_) {
		mydiskalloc_free(myhead_, node->chunk_head_);	
		node->chunk_head_ = mydiskalloc_alloc(myhead_, new_len);
		if(node->chunk_head_ == NULL_HANDLE) {
			free_list_insert(node);
			return NULL;
		}
	}*/
	...
	if(mydiskalloc_write(myhead_, node->chunk_head_, new_data, new_len) != 0) {
		mydiskalloc_free(myhead_, node->chunk_head_);
		free_list_insert(node);
...
THashNode* CHashMap::update_node_metadata(THashNode* node, int new_len, long expiretime)
{
#if 0
	int old_chunk_num = allocator_.get_chunk_num(node->chunk_len_);
	int new_chunk_num = allocator_.get_chunk_num(new_len);
...
int CHashMap::delete_node(THashNode* node, char* data, int* data_len)
{
	...
	if(data != NULL && data_len != NULL)
	{
		...
		*data_len = node->chunk_len_;
		if(mydiskalloc_read(myhead_, node->chunk_head_, data, node->chunk_len_) != 0)
		{
			return -1;
		}
	}
...
void CHashMap::use_node(THashNode *node, TMBHashKey &key, int chunk_len, DISK_HANDLE chunk_head)
{
	//set member
	node->key_ = key;
