...
int TFCDebugLog::shift_file(int cur_time)
{
	...
	time_t		l_cur_time = (time_t)cur_time;
	time_t		l_pre_time = (time_t)pre_time_;
	
    localtime_r(&l_cur_time, &cur_tm);
    localtime_r(&l_pre_time, &pre_tm);
...
void TFCDebugLog::log_p_ms(int log_level, const char* FMT, ...)
{
#ifndef WIN32
    MutexGuard g(p_mutex_);
#endif
    ...
    shift_file(tv.tv_sec);

    va_list ap;
    va_start(ap, FMT);
    log_i_v_ms(log_level, tv, FMT, ap);
    va_end(ap);
}
...
void TFCDebugLog::log_buf_p_ms(int log_level, const char *buf, int iLen)
{
#ifndef WIN32
	MutexGuard g(p_mutex_);
#endif
    ...
    gettimeofday(&tv, NULL);
	shift_file(tv.tv_sec);
	log_buf_i_ms(log_level, tv, buf, iLen);
}
...
char* TFCDebugLog::GetTimeFormat(size_t iTime, char* szDateTime)
{
	time_t	l_time = (time_t)iTime;
    struct tm stTm ;
    localtime_r(&l_time, &stTm);
...
char* TFCDebugLog::GetTimeFormatMS(const struct timeval &stTimeval, char* szDateTime)
{
...
void TFCDebugLog::log_i_v_ms(int log_level, const struct timeval &tv, const char *FMT, va_list ap)
{
    if (log_level < log_level_)   {  return; }
...
void TFCDebugLog::log_buf_i_ms(int log_level, const struct timeval &tv, const char *buf, int iLen)
{
    if (log_level < log_level_)   {  return; }
