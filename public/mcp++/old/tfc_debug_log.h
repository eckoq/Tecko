...
public:
    ...
    static char* GetTimeFormat(size_t iTime, char* szDateTime); // Sep 28th, uniqueruan.
    static char* GetTimeFormatMS(const struct timeval &stTimeval, char* szDateTime); // kendywang, 2015-11-15
...
protected:
    ...
    void log_i_v_ms(int log_level, const struct timeval &tv, const char* FMT, va_list ap);    //内部LOG函数
    void log_buf_i(int log_level, int *pt, const char *buf, int iLen); //内部LOG函数
    void log_buf_i_ms(int log_level, const struct timeval &tv, const char *buf, int iLen); //内部LOG函数
};


#define DEBUG_P TFCDebugLog::instance()->log_p
#define DEBUG_P_MS TFCDebugLog::instance()->log_p_ms
#define DEBUG_P_NO_TIME TFCDebugLog::instance()->log_p_no_time
#define DEBUG_BUF_P TFCDebugLog::instance()->log_buf_p
#define DEBUG_BUF_P_MS TFCDebugLog::instance()->log_buf_p_ms
