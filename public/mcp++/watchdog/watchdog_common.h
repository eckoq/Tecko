...
typedef struct watchdog_info {
    ...
    int             max_coredump_one_day;               // Process max coredump a day, in case too many coredumps fill up disk
    int             cmd_unnormally_exit_count[PROC_MAX_COUNT];  // Record the count of command line exit unnormally today

    int             reboot_normally_exited_proc;           // 1 denotes automatically rebooting of the normally exited process, 0 vice versa.
    ...
    list_head_t		normally_stopped_list;	            // Process normally quit.
    int normally_stopped_cnt;

	myhash_t		run_hash;							// Running process hash table.
	myhash_t		stopped_hash;						// Stopped process hash table.							// CCD main server IP.

    // fix bug of tapd:9704808
    int disable_oomkiller_kill_watchdog;
} watchdog_info_t;
